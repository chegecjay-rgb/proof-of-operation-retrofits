...
contract TimelockBatchSemanticRuntime {
    ...
    function emitBatchExecuted(
        DeterministicBatchTemporalTopology.TemporalOperation[] memory operations
    ) external returns (bytes32) {
        bytes32 batchRoot = deriveBatchRoot(operations);

        emit TimelockBatchExecuted(
            batchRoot,
            operations.length,
            block.timestamp
        );
